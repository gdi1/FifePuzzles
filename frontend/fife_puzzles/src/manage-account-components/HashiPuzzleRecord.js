...
import { CardContainer, InfoPart, PuzzlePart, Buttons, PuzzlePartv2 } from './RecordStyledComponents'
import MiniHashi from "./MiniHashi";

export default function HashiPuzzleRecord({ isActive, rating, puzzle, id, difficulty, handleButtonPress = () => { }, handleLoadComments, forCreated = true, date }) {
    return (
        <CardContainer>
            <InfoPart>
                <span style={{ fontSize: text_styles.resizbale_font.small_med, color: text_styles.colors.primary }}>ID: {id}</span>
                <br />
                <span style={{ fontSize: text_styles.resizbale_font.small_med, color: text_styles.colors.primary }}>Size: {puzzle[0].length}x{puzzle.length}</span>
                <br />
            ...
            <PuzzlePartv2>
                <MiniHashi puzzle={puzzle} puzzleHeight={puzzle.length} puzzleWidth={puzzle[0].length} />
            </PuzzlePartv2>
            <Buttons>
                {forCreated &&
                    <>
