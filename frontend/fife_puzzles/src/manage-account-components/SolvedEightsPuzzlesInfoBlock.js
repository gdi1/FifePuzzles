...
export default function SolvedEightsPuzzlesInfoBlock({ setCommentsScreenIsOpen, set_id_to_show_comments }) {
    ...
    const downloadFunction = async (skip) => {
        let response = await fetch(
            `${process.env.REACT_APP_BACKEND_URL}solved-puzzles-for-solvers/get_solved_eights_puzzles`,
            {
                method: "POST",
                headers: {
                ...
                (puzzlesDownloaded.length == 0 ?
                    ...
                    <ScrollView ref={ScrollViewRef} style={{ display: 'flex', flexDirection: 'column', height: 100 }} onScroll={handleScroll} scrollEventThrottle={0.01}>
                        {puzzlesDownloaded.map((elem) => {
                            return <EightsPuzzleRecord rating={elem.average_rating} date={elem.date_solved} forCreated={false} key={elem.eights_puzzles._id} id={elem.eights_puzzles._id} difficulty={elem.eights_puzzles.difficulty} puzzle={elem.eights_puzzles.values}
                                handleLoadComments={() => {
                                    console.log(elem)
