...
export default function CreatedHashiPuzzlesInfoBlock({ setCommentsScreenIsOpen, set_id_to_show_comments, setDeleteMessageModal, setDeleteFunctionModal, setDeleteContentModal, setConfirmDeleteModalIsOpen, setDeleteLoadingModal, setDeleteLoadingFinishModal, setDeletResponseModal }) {
    ...
    const downloadFunction = async (skip) => {
        let response = await fetch(
            `${process.env.REACT_APP_BACKEND_URL}created_puzzles/get_created_hashi_puzzles`,
            {
                method: "POST",
                headers: {
    ...
    const deleteSudokuFunction = async (requested_sudoku_id) => {
        let response = await fetch(
            `${process.env.REACT_APP_BACKEND_URL}created_puzzles/delete_created_hashi_puzzle`,
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                ...
                (puzzlesDownloaded.length == 0 ?
                    ...
                    <ScrollView ref={ScrollViewRef} style={{ display: 'flex', flexDirection: 'column', height: 100 }} onScroll={handleScroll} scrollEventThrottle={0.01}>
                        {puzzlesDownloaded.map((elem) => {
                            return <HashiPuzzleRecord isActive={elem.hashis.active} rating={elem.AVG} key={elem.hashis._id} id={elem.hashis._id} difficulty={elem.hashis.difficulty} puzzle={elem.hashis.values} handleButtonPress={() => {
                                ...
                                setDeleteContentModal(
                                    <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-evenly', width: 'flex', marginTop: margins.lrg }}>
                                        <MiniHashi puzzleHeight={elem.hashis.values.length} puzzleWidth={elem.hashis.values[0].length} puzzle={elem.hashis.values} gridStyleExtra={{ width: '36vh', height: '50vh', marginRight: margins.lrg }} numberStyleExtra={{ fontSize: text_styles.resizbale_font.small }} />
                                    </div>
                                )
                                setDeleteFunctionModal(() => async () => {
                                    setDeleteLoadingModal(true);
                                    let response = await deleteSudokuFunction(elem.hashis._id);
                                    if (response.status = "success") {
                                        setPuzzlesDownloaded((prev) => {
                                            return prev.filter(e => e.hashis._id != elem.hashis._id)
                                        })
                                    }
