...
export default function Puzzle8sMenuScreen() {
  ...
  const [isLoadingInModalFinished, setIsLoadingInModalFinished] =
    useState(false);
  const [loadingInModalMessage, setLoadingInModalMessage] = useState("");
  const [loadingInModalRedirectToGame, setLoadingInModalRedirectToGame] =
    useState(false);
  //Puzzle to upload
    ...
    if (filesContent.length == 1) {
      (async () => {
        ...
        let upload_result = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}eights_puzzle/add_8s_puzzle_from_upload/`,
          {
            method: "POST",
            headers: {
        ...
        console.log("iuhiweudhiweudhwiehd", upload_result);
            ...
            !loadingInModalRedirectToGame ? ( //If there is a problem with file, don't offer redirect
              <>
                ...
                <GeneralButton
                  ...
                  label={"OK"}
                  handleButtonPress={() => setFileUploadModalIsOpen(false)}
                />
              </>
            ) : (
              ...
              <>
                ...
                <GeneralButton
                  ...
                  label={"Go to game"}
                  handleButtonPress={() => {
                    navigate("/8s_puzzle/8s_puzzle_play", {
                      state: { game: puzzleFromFile },
                    });
                  }}
                />
              </>
            ...
            >
              ...
              <GeneralButton
                ...
                handleButtonPress={async () => {
                  console.log("iuewhdiuehwdihweidhwih")
                  setIsLoading(true);
                  //Getting puzzle from db
                  let result = await fetch(
                    `${process.env.REACT_APP_BACKEND_URL}eights_puzzle/get_eights_puzzle/`,
                    {
                      method: "POST",
                      headers: {
                        "Content-Type": "application/json",
